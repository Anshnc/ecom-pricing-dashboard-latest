-- Impact PP / GM: require quoted_pp for impact_pp_diff; calendar-yesterday-only prev NLC for deflection.
...
DECLARE
  ...
  prev_date date := p_delivery_date - interval '1 day';
BEGIN
  ...
  WHERE d.fsn_id = p_fsn_id
    ...
    AND ps.city = p_city
    AND ps.delivery_date = prev_date
  LIMIT 1;
...
BEGIN
  ...
  IF new.grn_price_per_unit IS NOT NULL
     AND new.demand_pct IS NOT NULL
     AND new.quoted_pp IS NOT NULL THEN
    new.impact_pp_diff := round((new.quoted_pp - new.grn_price_per_unit) * new.demand_pct / 100, 2);
  ELSE
