-- NLC is null until Quoted PP is entered. Negotiated PP never affects grid NLC.
-- GM, PI%, deflection, and impact metrics follow the same rule.
...
AS $$
  SELECT CASE
    WHEN p_quoted_pp IS NULL THEN NULL
    ELSE p_quoted_pp
...
BEGIN
  ...
  IF new.quoted_pp IS NOT NULL THEN
