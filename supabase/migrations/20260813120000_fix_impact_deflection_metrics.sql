-- Fix impact metrics (/100 on demand_pct), cost backfill from fsn_cost_components,
-- and recompute derived columns on existing rows.
...
begin
  ...
  -- Fixed costs from reference table when missing or zero.
...
-- Backfill costs + derived metrics on existing detail rows.
update public.price_sheet_details d
set
  pm_cost = coalesce(nullif(scc.pm_cost, 0), nullif(d.pm_cost, 0), 0),
  fml_dump = coalesce(nullif(scc.fml_dump, 0), nullif(d.fml_dump, 0), 0),
  pc = coalesce(nullif(scc.pc, 0), nullif(d.pc, 0), 0)
from public.fsn_cost_components scc
where scc.fsn_id = d.fsn_id
  and scc.weight_unit = d.weight_unit
  and (
...
-- Touch rows so BEFORE UPDATE trigger recomputes nlc / deflection / impact columns.
